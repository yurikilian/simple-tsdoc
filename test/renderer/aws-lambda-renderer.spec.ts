...
import AwsLambdaRenderer from '../../src/renderer/AwsLambdaRenderer';
import { GlobalDocumentation } from '../../src/renderer/Renderer';

it('Should render mustache file', () => {
  ...
  const documented: { [key: string]: any } = {
    functions: [],
    interfaces: []
  };
  if (rootSourceFile) {
  ...
  const renderer = new AwsLambdaRenderer({
    outputPath: path.join(__dirname, '../fixture/renderer/out', 'index.md'),
    templatePath: path.join(__dirname, '../fixture/renderer', 'template.md')
  });

  renderer.render(documented as GlobalDocumentation);
});
